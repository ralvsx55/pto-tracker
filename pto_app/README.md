...
2. First time, or whenever you want a clean database:
   ...
   - creates admin `chris@lightsaberpromotions.com` / `changeme-now`, viewer passwords `staff` for both groups
...
| auth.php | `current_user`, `require_login`, `require_role`, `login`, `logout`, `user_can_group`, `user_create` |
...
| viewer_auth.php | `viewer_trusted`, `viewer_issue_cookie`, `viewer_check_password`, `viewer_set_password`, `request_is_https` |
